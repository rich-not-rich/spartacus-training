'use strict';
Object.defineProperty(exports, '__esModule', { value: true });
const constants_1 = require('../../../../shared/constants');
exports.CHECKOUT_DELIVERY_SERVICE_MIGRATION = {
  // projects/core/src/checkout/facade/checkout-delivery.service.ts
  class: constants_1.CHECKOUT_DELIVERY_SERVICE,
  importPath: constants_1.SPARTACUS_CORE,
  deprecatedParams: [
    {
      className: constants_1.STORE,
      importPath: constants_1.NGRX_STORE,
    },
    {
      className: constants_1.CART_DATA_SERVICE,
      importPath: constants_1.SPARTACUS_CORE,
    },
  ],
  removeParams: [
    {
      className: constants_1.CART_DATA_SERVICE,
      importPath: constants_1.SPARTACUS_CORE,
    },
  ],
  addParams: [
    {
      className: constants_1.AUTH_SERVICE,
      importPath: constants_1.SPARTACUS_CORE,
    },
    {
      className: constants_1.ACTIVE_CART_SERVICE,
      importPath: constants_1.SPARTACUS_CORE,
    },
  ],
};
//# sourceMappingURL=checkout-delivery.service.migration.js.map
